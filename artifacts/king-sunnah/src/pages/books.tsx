...
function BookCard({ book, isMatn }: { book: MutoonBook | ServiceBook; isMatn: boolean }) {
  ...
  return (
    <div className="surface-card group flex gap-4 p-4 transition-transform duration-300 hover:-translate-y-0.5 sm:gap-5 sm:p-5">
      <Link href={`/book/${id}`} className="w-24 flex-shrink-0 sm:w-28">
        <BookCoverArt title={book.title} author={book.author} imageSrc={coverImageForTitle(book.title)} className="rounded-lg" />
      </Link>

      <div className="flex min-w-0 flex-1 flex-col">
        <div>
          <Link href={`/book/${id}`}>
            <h3 className="truncate font-display text-lg font-medium transition-colors hover:text-primary">
              {book.title}
            </h3>
          </Link>
        ...
        <div className="mt-auto flex items-center gap-2 pt-4">
          ...
          <Link
            href={`/book/${id}`}
            className="mr-auto inline-flex h-9 items-center justify-center rounded-full border border-border px-4 text-sm font-medium transition-colors hover:bg-foreground/[0.04]"
          >
            عرض التفاصيل
          </Link>
        </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as BookTab)}>
          <TabsList className="h-11 w-full gap-1 rounded-full bg-foreground/[0.05] p-1 sm:w-auto">
            <TabsTrigger
              value="mutoon"
              className="h-9 flex-1 gap-1.5 rounded-full px-4 text-sm data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-none sm:flex-none"
            >
              كتب المتون
